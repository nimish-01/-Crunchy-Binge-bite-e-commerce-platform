...
const damageSchema = z.object({
  ...
  notes: z.string().min(1, "Notes are required for damage records"),
})
  ...
  try {
    ...
    if (variant.stock < quantity) {
      return NextResponse.json(
        { success: false, error: `Cannot mark ${quantity} units as damaged. Only ${variant.stock} in stock.` },
        { status: 400 }
      )
    }
    ...
    return NextResponse.json({
      success: true,
      data: result,
      message: `Marked ${quantity} units as damaged. New stock: ${result.variant.stock}`,
    })
  } catch (error) {
    console.error("[POST /api/inventory/damage]", error)
